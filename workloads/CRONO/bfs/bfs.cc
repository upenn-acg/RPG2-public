...
void* do_work(void* args)
{
   ...
   for (int i = 0; i < iters; i++) {
     ...
     //std::chrono::system_clock::time_point iterStart = std::chrono::system_clock::now();

     iter = 0;
     
     // set terminate, Q, D, temporary
     ...
     //std::chrono::system_clock::duration elapsed = std::chrono::system_clock::now() - iterStart;
     //using date::operator<<;  // Needed to find the correct operator<<
     //std::cout << elapsed << '\n';
     
     //following three lines added by soyoon
     //clock_gettime(CLOCK_REALTIME, &iterEnd);
     //double iter_accum = ( iterEnd.tv_sec - iterStart.tv_sec ) + ( iterEnd.tv_nsec - iterStart.tv_nsec ) / BILLION;
     //fprintf(file1, "%d\t%lf\n", i,iter_accum );
  }
   //added by soyoon
   //fclose(file1);
...
int main(int argc, char** argv)
{
  ...
  clock_gettime(CLOCK_REALTIME, &initStart);
   ...
   if(select==1)
   {
      ...
      //added by soyoon 
      inputpointer = basename(filename);     
 ...
 /*
      ...
      char input[strlen(filename) - len];
      int counter = 0;
      inputpointer = input;
      while (len < strlen(filename)) {
         input[counter] = filename[len];
         counter++;
         len++;
      }
...
*/
   ...
   double accum0 = ( initEnd.tv_sec - initStart.tv_sec ) + ( initEnd.tv_nsec - initStart.tv_nsec ) / BILLION;
   printf( "\nInitialization Time:%lf seconds\n", accum0 );
   ...
   for(int j = 0; j < P; j++) {
      ...
      thread_arg[j].iters = iters;
      thread_arg[j].input = inputpointer;
   }
