...
def generate_scatter_plot(abs_dist, normalized_runtime):
    plt.figure(figsize=(10, 6))
    num_bins = 20
    #plt.scatter(percentage_dist, normalized_runtime, marker='o', alpha=0.5)
    plt.hist(abs_dist, num_bins, edgecolor='black')
    plt.xlabel('Absolute Distance from Optimal Prefetch Distance')
    #plt.ylabel('PG2 Speedup')
    plt.ylabel('Frequency')
    #plt.title('Speedup vs Distance from Optimal Prefetch Distance on all Single Optimal Inputs')
    plt.title('Distance from Optimal Prefetch Distance on all Single Optimal Inputs')
    #plt.grid(True)
    plt.savefig("/home/nsobotka/floar/pg2_reloc/results/distFromOpt/distFromOpt.png")
    print("Saved figure at /home/nsobotka/floar/pg2_reloc/results/distFromOpt/distFromOpt.py")
