...
typedef struct func_info{
   ...
   vector<pair<int,int>> prefetch_locations;
} func_info;
...
typedef struct IPC_info{
   ...
   IPC_info(double ipc, int pref_dist){
      IPC = ipc;
      prefetch_distance = pref_dist;
   }
} IPC_info;
 ...
 */
typedef struct pg2_env{
   unordered_map<string, string> configs;

   string dir_path; 
   string target_bin_path;
   string target_bin_name;
   string bolt_opt_bin_path;
   ...
   string prefetch_insn_bin;
   string tmp_prefetch_insn_bin;
