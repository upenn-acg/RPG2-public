...
void thread_function(vector<pair<long, long>> func_name, unordered_map<long, call_site_info> &call_sites, string new_target_binary, pg2_env* pg2_environ){
   for (unsigned i=0; i<func_name.size(); i++){
      ...
      string command = ""+pg2_environ->objdump_path+" -d --start-addr=0x" + start_addr + " --stop-addr=0x" + stop_addr + " " + new_target_binary;
...
int main(){
   ...
   char path3[1000];
   pg2_env pg2_environ;
   string command = "cp "+pg2_environ.target_bin_path+" "+pg2_environ.tmp_data_path;
   cout<<"##### cmd= "<<command<<endl;
   system(command.c_str());

   vector<string> dir_names = split(pg2_environ.target_bin_path, '/');
   string new_target_binary = pg2_environ.tmp_data_path + dir_names[dir_names.size()-1];

   command = "strip -g "+new_target_binary;
   system(command.c_str());

   command = ""+pg2_environ.nm_path+" -S -n "+new_target_binary;	
   char* command_cstr = new char[command.length()+1];
   strcpy (command_cstr, command.c_str());
   ...
   for (int i=0; i<N; i++){
      thread t(thread_function, func_names[i], ref(call_sites_array[i]), new_target_binary, &pg2_environ);		
   ...
   ofstream ofs(pg2_environ.call_sites_all_bin);
   ...
   ofstream ofs1(pg2_environ.call_sites_list_bin);
